...
const Post = () => {
  ...
  useEffect(() => {
    const fetchPostAndComments = async () => {
      try {
        const postResponse = await axios.get(`http://localhost:5001/posts/${id}`);
        setPost(postResponse.data);

        const commentsResponse = await axios.get(`http://localhost:5001/posts/${id}/comments`);
        setComments(commentsResponse.data);
      } catch (err) {
        setError("Error fetching post or comments.");
        console.error(err);
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      setComment("");
      const updatedComments = await axios.get(`http://localhost:5001/posts/${id}/comments`);
      setComments(updatedComments.data);
    } catch (err) {
      console.error("Error submitting comment:", err);
      setError("Error submitting comment.");
